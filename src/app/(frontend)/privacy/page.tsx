...
export default async function PrivacyPage() {
  ...
  return (
    <article className="container max-w-3xl pt-24 pb-24 prose prose-neutral dark:prose-invert">
      ...
      <ul>
        <li>
          Поисковые запросы (текст), который вы вводите в AI-помощник
          или в строку поиска. Сохраняются вместе с email-логином, чтобы
          учитывать предыдущие запросы при следующей сессии.
        </li>
        <li>
          Координаты вашего местоположения — только если вы явно
          разрешили геолокацию в браузере при клике «учитывать
          моё местоположение».
        </li>
        <li>
          Список избранных и недавно просмотренных объектов — хранятся
          в localStorage браузера, передаются в рекомендатель только в
          моменте запроса.
        </li>
        <li>
          Запросы (без email и других персональных данных) отправляются
          на сервер языковой модели (Anthropic Claude или локальный
          сервис) для извлечения структурированных фильтров и
          генерации объяснений. На стороне поставщика хранятся
          в соответствии с их политикой.
        </li>
      </ul>
      <p>
        <strong>Как отказаться:</strong> не используйте AI-помощник
        (иконку <em>спаркл</em> в шапке и кнопку «Подобрать»). Обычный
        поиск работает без обработки персональных данных в LLM.
      </p>
      <p>
        Чтобы удалить накопленную историю поисков, напишите запрос на
        email оператора (см. раздел 9).
      </p>
